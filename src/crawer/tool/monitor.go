...
// level 日志级别: 0:普通 1：警告；2：错误
type Record func(level byte, content string)

// 调度器监控函数
// scheduler代表作为监控目标的调度器
// intervalNs检查间隔时间，单位：纳秒
// maxIdleCount代表最大空闲计数
// autoStop是否在调度器持续空闲一段时间后（intervalNs * maxIdleCount得出）之后自行停止调度器
// detailSummary被用来表示是否需要详细的摘要信息
// record代表日志记录函数
// 监控结束后，该方法向通道发送一个代表空闲检查次数的数值
func Monitoring(
